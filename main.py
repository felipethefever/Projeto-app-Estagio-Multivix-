...
def extract_data(carro, pages, subject_pattern, price_pattern, title_pattern):
...
# Define os padrões de extração
subject_pattern = re.compile(r'Ano","value":"(.*?)"},')
price_pattern = re.compile(r'price":"(.*?)",')
title_pattern = re.compile(r'title":"(.*?)",')

# Chamar a função
df = extract_data(carro, 5, subject_pattern, price_pattern, title_pattern)
